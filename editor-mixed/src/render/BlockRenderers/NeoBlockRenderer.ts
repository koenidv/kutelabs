...
export class NeoBlockRenderer extends BaseBlockRenderer {
  ...
  private addContainerNooks(
    ...
    connectors.forEach(connector => {
      ...
      const upstreamConnected = block.upstream?.connectedBlocks.byBlock(block)
      if (connector.type === ConnectorType.Before && upstreamConnected) {
        if (
          upstreamConnected.type === ConnectorType.Inner &&
          upstreamConnected.role === ConnectorRole.Input
        )
          return
        if (upstreamConnected.role === ConnectorRole.Conditional) return
        if (upstreamConnected.role === ConnectorRole.Output) return
      }
  ...
  protected override renderContentFunction(
    ...
    const head = svg`
      ...
      <text x=${size.fullWidth - size.rightWidth - 112} y=${size.fullHeight - size.fullTailHeight / 2} text-anchor="end" alignment-baseline="middle" opacity="1" fill="white">returns</text>
      <g transform="translate(${size.fullWidth - size.rightWidth - 58}, ${size.fullHeight - size.fullTailHeight / 2 - 16}) rotate(90) scale(0.1, 0.125)" fill="white">
